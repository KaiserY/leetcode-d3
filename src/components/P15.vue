...
onMounted(() => {
  ...
  setInterval(() => {
    ...
    if (left < right) {
      if (input[left] + input[right] == -input[start]) {
        left = alignLeft(input, left);
        right = alignRight(input, right, start);
      } else if (input[left] + input[right] > -input[start]) {
        right = alignRight(input, right, start);
      } else {
        left = alignLeft(input, left);
      }
      ...
      if (
        input[left] + input[right] == -input[start] &&
        left != right &&
        left != start &&
        start != right &&
        left < right
      ) {
        output[outputIndex][0] = input[start];
        output[outputIndex][1] = input[left];
        output[outputIndex][2] = input[right];
...
function alignLeft(values: any[], left: number): number {
  var ret = left + 1;

  while (ret < values.length && values[ret] == values[ret - 1]) {
    ret += 1;
  }
...
function alignRight(values: any[], right: number, start: number): number {
  var ret = right - 1;
